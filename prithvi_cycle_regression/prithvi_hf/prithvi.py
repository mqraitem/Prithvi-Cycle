...
class PrithviSeg(nn.Module): 
    def __init__(self,
        ...
        if model_size == "300m":
            self.head = nn.Sequential(
                ...
                nn.Conv2d(in_channels=256, out_channels=n_classes * prithvi_params["num_frames"], kernel_size=1),
            )
    ...
    def forward(self, x):
        ...
        x = self.backbone(x)
        x = self.head(x)
        x = x.view(batch_size, self.n_frames, self.n_classes, x.size(2), x.size(3))   
        return x
