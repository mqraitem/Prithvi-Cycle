...
def convert_final_mask_to_signed_distance(final_mask, last_month):
    # Ensure last_month is valid
    assert 0 <= last_month <= 11, "last_month should be between 0 and 11"

    # Initialize output array
    distance_mask = np.copy(final_mask)  # Preserve original structure

    # Compute signed distance where final_mask is NOT -1
    valid_mask = final_mask != -1  # Boolean mask for valid pixels
    distance_mask[valid_mask] = final_mask[valid_mask] - last_month  # Compute signed difference
    # distance_mask[valid_mask] += 11  # Shift values to range [0, 22]
...
class cycle_dataset(Dataset):
	def __init__(self,path,split, region):
		self.data_dir=path
		self.split=split
		self.region = region
	...
	def get_means_stds(self):
		...
		means_stds_path = "/usr4/cs505/mqraitem/ivc-ml/geo/data/Northeast/means_stds_distance.pkl"
	...
	def fix_final_mask(self,final_mask, last_month):	
		...
		final_mask = np.where(mask, -1, final_mask)
		final_mask_distance = convert_final_mask_to_signed_distance(final_mask, last_month)

		return final_mask, final_mask_distance
	
	def __getitem__(self,idx):
		
		image_path=self.data_dir[idx][0]
		mask_path=self.data_dir[idx][1]
		last_month=self.data_dir[idx][2] 
		hls_tile_name = self.data_dir[idx][3]
		...
		final_mask, final_mask_distance = self.fix_final_mask(final_mask, last_month)
		...
		return {
			"image": final_image,
			"mask": final_mask,
			"mask_distance": final_mask_distance,
