...
def eval_data_loader(data_loader,model, device):
	...
	for hls_tile_n in tqdm(all_out):
		...
		total_correct = compute_accuracy(mask_hls_tile, predicted_month, total_correct)
...
def main():
	...
	if args.logging: 
		wandb.init(
				project="Geospatial_Distance_final",
				group=group_name,
				config = wandb_config, 
				name=wandb_name,
				)
	...
	for epoch in range(config["training"]["n_iteration"]):
		...
		for j,batch_data in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
			...
			mask = batch_data["mask_distance"]	
			class_weights = batch_data["class_weights"]	
